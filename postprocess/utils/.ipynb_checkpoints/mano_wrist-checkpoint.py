...
"""
    @date:    2022.04.07  week15  星期四
    @func:    mano_wrist模型的变形代码.
    
    @update:  2022.05.11  week20  星期三
    @func:    lixin对齐.
"""
...
kintree_table = dd['kintree_table']  # 不用改
id_to_col = {kintree_table[1, i]: i for i in range(kintree_table.shape[1])}  # 不用改
parent = {i: id_to_col[kintree_table[0, i]] for i in range(1, kintree_table.shape[1])}  # 不用改

mesh_mu = verts.unsqueeze(0)  # [bs, 778, 3] ---> [bs, 842, 3]改成verts即可(hand_01/02)
...
hands_mean = Variable(torch.from_numpy(np.expand_dims(dd['hands_mean'], 0).astype(np.float32)).cuda()) # [bs, 45] 这个不变
root_rot = Variable(torch.FloatTensor([0., 0., 0.]).unsqueeze(0).cuda())
# FACES = dd['f']  #  faces[0]的numpy格式即可.
FACES = faces[0]  #  faces[0]的numpy格式即可.

# 设置64个额外vertex的J_regressor & posedirs & weights
# 策略1:  J_regressor & posedirs & weights 等于离其最近的值.
# 策略2:  posedirs的手腕部分都是0 & weights 的手腕部分第1项都是1 (果然是weights的问题!)
